...
function renderRows(items) {
  if (!items || items.length === 0) {
    ledgerBody.innerHTML = '<tr class="empty-row"><td colspan="7">No items currently clear both thresholds. Check back after the next sync.</td></tr>';
    return;
  }
...
async function refresh() {
  try {
    ...
    ledgerBody.innerHTML = '<tr class="empty-row"><td colspan="7">' +
